...
if st.button("Send"):
    if user_input:
        ...
        if orchestrator_response == "out-of-scope":
            ...
            for property in current_listings_data_response["return_data"]:
                with st.container():
                    cols = st.columns(2)
                    with cols[0]:
            ...
            st.session_state['chat_history'].append({"role": "assistant", "content": current_listings_data_response["return_str"]})
        else:
